...
const postModule = {
  ...
  getters: {
    ...
    sortedAndSearchedPosts(state, getters, { coomit }) {
      console.log(getters.sortedPost);
      ...
      // commit('setTotalPage', Math.ceil(data.length / state.limit));
      return getters.sortedPost.filter((post) => String(post[state.selectedSort]
        ?? post.user.login).toLowerCase().includes(state.searchQuery.toLowerCase()));
    },
  },
  mutations: {
