...
const postModule = {
  state: () => ({
    ...
    sortOptions: [
      ...
      { value: 'userId', name: 'По авторам' },
    ],
  }),
  getters: {
    sortedPost(state) {
      return [...state.posts].sort((post1, post2) => String(post1[state.selectedSort])
    ...
    sortedAndSearchedPosts(state, getters) {
      return getters.sortedPost.filter((post) => {
        ...
        return String(post[state.selectedSort]) .toLowerCase().includes(state.searchQuery.toLowerCase());
      });
    },
  },
  mutations: {
