...
const postModule = {
  ...
  actions: {
    async fetchPost(dispatch, { commit }) {
      try {
        commit('setLoading', true);
        const response = await axios.get('http://lcalhost:3001/api/news');
        commit('setPosts', [...response.data]);
      } catch (err) {
        // commit('setError', err);
        dispatch('dialog/setDialogVisibleError', true, { root: true });
        // commit('dialogModule/setDialogVisibleError', true);
      } finally {
