...
const authModule = {
  ...
  actions: {
    ...
    //     const response = await axios.get('http://localhost:3001/api/auth/whoami', {
    //       headers: {
    ...
    async userSign({ commit, state }, { _login = null, _email, _password }) {
      try {
        ...
        commit('setLoading', true);
        if (state.isSignUp) {
          response = await api.post('http://localhost:3001/api/auth/signup', {
            login: _login,
            email: _email,
            password: _password,
          });
          ...
          response = await api.post('http://localhost:3001/api/auth/signin', {
            email: _email,
            password: _password,
          });
