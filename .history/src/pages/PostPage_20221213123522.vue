...
export default {
  ...
  beforeMount() {
    console.log('PostPage beforeMount');
    this.fetchPost();
  },
  computed: {
    ...mapState({
  ...
  updated() {
    console.log('updated');
    console.log(this.sortedAndSearchedPosts.length);
    if (this.page > this.sortedAndSearchedPosts.length) {
      this.setPage(Math.ceil(this.sortedAndSearchedPosts.length / this.limit));
    }
  },
};
</script>
