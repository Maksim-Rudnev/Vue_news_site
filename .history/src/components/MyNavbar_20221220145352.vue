<template>
  <div class="navbar">
    ...
    <div class="right-panel">
      <my-button v-if="!isAuth" @click="setVisible(true), setType('SIGNUP')">
      Регистрация
    </my-button>
    <my-button id="login-user"
    v-else @click="$router.push(`/user/${id}`)">
      {{login}}
    </my-button>

    <my-button v-if="!isAuth" @click="setVisible(true), setType('SIGNIN')">
      Войти
    </my-button>
  ...
  <my-dialog v-if="this.type === 'SIGNUP' || this.type === 'SIGNIN'"
      :show="visible"
